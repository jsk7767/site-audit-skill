...
## `this_week` — 이번 주에 할 일 하나 (권장)

보고서 맨 위, 점수 카드 바로 아래에 크게 뜬다. 사장님이 첫 화면에서 무엇부터 할지 알게 하는 자리다.

```jsonc
"this_week": {
  "title":  "이번 주에 끝낼 수 있는 한 가지",
  "why":    "왜 이것이 먼저인가. 다른 것보다 먼저인 이유를 근거로 적는다",
  "how":    "비전문가가 그대로 따라 할 수 있는 절차. 복사해 쓸 문장이나 값이 있으면 여기에",
  "effort": "예상 상승폭 · 작업량",
  "owner":  "누가 하는가 (선택). 매장 확인이 필요하면 그 사실을 적는다"
}
```

**규칙**
- 이번 주 안에, 비전문가가 할 수 있는 것이어야 한다. 개발자가 하루 붙어야 하는 일은 여기 오지 않는다.
- `levers[0]` 과 같아도 된다. 비워 두면 `levers[0]` 이 자동으로 들어간다 (지어내지 않는다).
- 확인되지 않은 사실(이름·경력·가격)을 실행 절차에 넣지 않는다. 확인이 필요하면 `owner` 에 그렇게 적는다.
- 이 절은 나머지 목록을 대체하지 않는다. 전수 점검 결과는 아래에 그대로 남는다.
