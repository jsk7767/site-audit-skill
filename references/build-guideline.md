...
나머지 절은 이 다섯 가지를 항목으로 푼 것이다. 시간이 없으면 이것만 지킨다.
...
## AI 로 홈페이지를 만들 때 넣을 지시문

요즘은 사람이 코드를 치지 않고 AI 에게 시켜서 짓는 경우가 많다. 그때 아래를 그대로 붙여 넣는다.
여기 있는 문장은 전부 **나중에 고치기 비싼 것들**이다. 다 짓고 나서 발견하면 구조를 다시 짜야 한다.

```text
아래 조건을 지켜서 만들어 줘.

[페이지 구조]
- 다루는 주제(장소·서비스·메뉴 등)마다 독립 페이지를 만든다. 한 페이지는 한 가지 질문에만 답한다.
- 각 페이지에는 목록이나 사진뿐 아니라 그 주제에 대한 본문 글이 들어갈 자리를 둔다.
- 태그·카테고리로 자동 생성되는 목록 페이지는 사이트 안에서 골라 보는 용도로만 쓰고
  <meta name="robots" content="noindex"> 를 넣는다. 사이트맵에도 넣지 않는다.

[head — 가장 중요]
- title, meta description, canonical, og:title, og:description, og:url, og:image 를
  페이지마다 개별로 설정할 수 있게 한다. 공통 레이아웃이나 템플릿 head 에 값을 문자열로 박지 않는다.
- canonical 은 그 페이지 자기 자신을 가리킨다. og:url 도 canonical 과 같은 값이어야 한다.
- 다국어면 hreflang 을 모든 언어판에 상호 참조로 넣고, 같은 언어의 정본은 하나만 둔다.

[본문]
- 본문은 서버에서 렌더된 HTML 에 들어 있어야 한다. 자바스크립트로만 그리면 안 된다.
- 제목 단계(h1→h2→h3)를 건너뛰지 않는다. 글자 크기는 CSS 로 정하고 단계는 내용의 층으로 정한다.
- h1 은 페이지마다 하나만 둔다.
- 숫자(가격·시간·거리·인원), 정의 문장, 비교표, 번호 매긴 절차를 넣을 수 있는 구조로 만든다.

[이미지]
- 파일명을 내용에 맞게 지을 수 있게 하고, 관리 화면에서 이미지마다 alt 를 입력할 수 있게 한다.
  alt 입력란은 지금 넣는다. 나중에 수천 장을 소급 입력하는 것보다 훨씬 싸다.
- WebP 로 내보내고 width/height 를 지정해 로딩 중 레이아웃이 밀리지 않게 한다.
- 화면 밖 이미지는 loading="lazy", 첫 화면 대표 이미지는 lazy 를 걸지 않는다.

[검색·AI 접근]
- robots.txt 에서 검색 봇과 AI 검색 봇을 막지 않는다.
- nosnippet, max-snippet:0, noarchive 를 넣지 않는다. 넣으면 색인은 되지만 AI 답변 인용에서 빠진다.
- sitemap.xml 을 자동 생성하고 robots.txt 에 위치를 적는다.
- JSON-LD 를 넣되 화면에 실제로 있는 정보만 넣는다. 전화·주소·영업시간은 화면 표기와 같은 값이어야 한다.
- 평점(aggregateRating)을 임의로 만들지 않는다.

[화면]
- 글자와 배경 대비를 본문 4.5:1, 큰 글씨 3:1 이상으로 한다.
- 아이콘만 있는 버튼에 aria-label, 입력칸에 label 을 붙인다.
- 한국어 본문에 word-break: keep-all 을 적용해 단어 중간에서 줄이 꺾이지 않게 한다.
- 링크와 버튼의 터치 영역을 44x44 이상으로 한다.
```
...
| 지시문 | 안 지키면 | 대응 룰 |
|---|---|---|
| head 를 페이지 변수로 | 전 페이지가 같은 제목·canonical 이 되어 색인이 무너진다 | `S-T-canonical-collapse` P0 · `S-O-title-dup` P0 · `S-O-meta-hardcoded` |
| 태그 목록은 noindex | 얇은 페이지가 태그 수만큼 생기고 본문이 서로 같아진다 | `S-C-thin` · `S-C-dup-content` |
| 한 페이지 한 질문 | 인용·색인의 최소 단위가 URL 인데 한 장에 여러 주제가 섞인다 | `S-O-title-dup` · `G-C-lead` |
| 서버 렌더 | 자바스크립트를 실행하지 않는 수집기에는 빈 페이지로 보인다 | `G-A-ssr` |
| alt 입력란을 지금 | 사진이 쌓인 뒤 소급 입력은 사실상 안 된다 | `S-I-alt` |
| nosnippet 금지 | 색인은 되는데 AI 답변의 인용 후보에서 빠진다 | `S-T-snippet-block` |
| 스키마와 화면 일치 | 사람이 볼 수 없는 정보는 확인할 수 없는 주장이 된다 | `S-S-drift` |
| 제목 단계 | 기계가 본문을 덩어리로 끊어 읽는 경계가 어긋난다 | `S-O-heading-order` |
| 대비·이름 | 사람에게도 화면을 대신 읽는 기계에게도 안 보인다 | `D-a11y-contrast` · `D-a11y-name` |

만든 뒤에는 배포 전 게이트로 확인한다. `run_all.py --local <빌드폴더> --base <공개 URL>` 이 exit 0 이어야 한다.
...
### AI 검색 유입은 GA4 의 AI Assistant 채널로 본다

AI 답변에서 실제로 몇 명이 넘어왔는지는 오래 측정이 안 됐다. GA4 가 기본 채널 그룹에 **AI Assistant** 를 추가하면서 이것만은 결정적으로 볼 수 있게 됐다.

- 조건: 매체(medium) `ai-assistant` · 캠페인 `(ai-assistant)`. 리퍼러가 인식된 AI 서비스면 GA4 가 자동으로 이 값을 넣는다.
- 인식 목록: 출시 공지에는 ChatGPT·Gemini·Claude 가, 2026년 6월 문서에는 ChatGPT·Gemini·Deepseek·Copilot·Grok 이 적혀 있다. **구글이 전체 목록을 공개하지 않았고 두 표기가 다르므로, 이 목록을 확정으로 쓰지 않는다.**
- **한계가 크다.** 리퍼러 헤더 없이 오는 유입은 전부 Direct 로 빠진다. 앱 안 브라우저, 모바일 앱, 링크 복사·붙여넣기가 여기 해당한다. 즉 **AI Assistant 수치는 하한선이지 전부가 아니다.** 보고서에 이 문장을 함께 적는다.

운영 루틴에 넣을 것: 월 1회 GA4 → 획득 → 트래픽 획득 → 기본 채널 그룹에서 AI Assistant 행의 **세션 수와 참여율**을 기록한다. 절대값보다 같은 사이트의 전월 대비가 의미 있다.

### 트래픽 숫자를 성과로 읽기 전에 세 가지를 본다

세션 수만 보면 봇을 사람으로 착각한다. 실제로 흔히 벌어지는 일이다.

| 볼 것 | 사람 유입이면 | 봇이 섞였으면 |
|---|---|---|
| 참여율 | 보통 40~60% | 1% 안팎 |
| 평균 참여시간 | 수십 초 이상 | 0초 |
| 그래프 모양 | 오르내리며 계단식 상승 | 수직 상승 후 평평 |

GA4 는 알려진 봇만 거른다. 헤드리스 브라우저·스크래퍼는 사람으로 센다.
**가장 확실한 대조는 서치콘솔이다.** 구글이 자기 검색 결과에서 센 클릭 수라 봇이 섞이지 않는다. GA4 Organic 세션과 서치콘솔 클릭이 자릿수 단위로 벌어지면 GA4 쪽을 의심한다.
