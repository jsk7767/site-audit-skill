...
- **판단 적용 여부가 다른 감사끼리 비교하면 안 된다.** 한쪽만 오탐을 걸러낸 상태면 점수도 해소·신규 목록도 기준이 다르다. `scores.json` 의 `narrative_applied` 가 서로 다르면 `site_compare.py` 가 델타 맨 위에, 보고서 비교표 위에 경고를 넣는다. 양쪽 모두 `--report-only` 로 판단을 적용한 뒤 다시 비교한다.
