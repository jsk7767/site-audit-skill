...
## 2026-09-05 전체 화면 캡처 순서 수정

진단 중 발견. 긴 페이지의 `*_full.png` 가운데가 통째로 비어 나왔다. 사이트 결함으로 오해할 뻔했다.

**원인**: `measure()` 가 전체 캡처를 **지연 로딩 스크롤 전에** 찍었다. lazy 이미지는 화면에 들어와야 뜨므로, 스크롤하지 않은 상태에서 `full_page=True` 로 찍으면 접힘 아래가 비어 있다.

**수정**: 접힘 위 캡처는 그대로 스크롤 전(손님이 처음 보는 화면), 전체 캡처는 스크롤이 끝난 뒤로 옮겼다. 찍기 전에 `window.scrollTo(0,0)` + 700ms 로 맨 위에 되돌린다(고정 헤더가 중간에 겹쳐 박히는 것 방지).

**검증**: 같은 페이지(13,830px 세로) 재캡처 — 631KB → 3,442KB. 육안 확인 결과 사진·섹션·메뉴·FAQ 가 전부 채워졌고 빈 구간이 사라졌다.

## 2026-09-05 점수 비교 함정 — 도구가 경고하게 함

실사용 중 발견. 안국약방 09-04 보고서의 SEO 97 · GEO 99 는 **판단 적용 후** 값이고, 09-05 재진단의 99 · 98 은 **적용 전** 기계 판정이었다. 나란히 놓으니 GEO 가 내려간 것처럼 보였다. 기계 판정끼리 비교하면 96→99, 97→98 로 실제로는 올랐다.

점수만이 아니라 **해소·신규 목록도 같은 함정**이다. 판단으로 PASS/HOLD 가 된 항목은 FAIL 집합에서 빠지므로, 다음 회차에 그대로 두면 "신규" 로 나타난다.
